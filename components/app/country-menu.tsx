...
export function CountryMenu({
  ...
  return (
    <div ref={rootRef} className="relative min-w-0 sm:w-auto">
      <button
        type="button"
        aria-haspopup="listbox"
        aria-expanded={open}
        onClick={() => setOpen((next) => !next)}
        className="app-input flex w-full cursor-pointer items-center gap-2 py-2 pr-2 text-left sm:w-auto"
      >
        {selected ? (
          <CountryFlag
            code={selected.code}
      ...
      {open ? (
        <div className="absolute left-0 top-full z-30 mt-1.5 w-[min(22rem,calc(100vw-2rem))] max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-app-line bg-app-surface shadow-2xl">
